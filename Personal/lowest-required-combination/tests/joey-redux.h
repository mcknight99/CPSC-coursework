...
std::vector<int> findLowestCombination_JR(const std::vector<int> &requiredInts, const std::vector<int> &supplementaryInts, int desiredModulo, int modulus)
{
    ...
    for (int i = nearestRSumMultiple; i < maxSum; i += modulus) //did not make as much as a difference as i was expecting really. probably would at large sizes.
        ...
        if (i % modulus == desiredModulo)
        {
            for (int j = 0; j < supplementaryInts.size(); j++)
            {
                if (canSumToTarget(i - minSum, supplementaryInts))
                {
                    ...
                    std::vector<int> lowestParts = subsetSum(supplementaryInts, i - minSum); //only takes about 7000ns, hardly noticeable
